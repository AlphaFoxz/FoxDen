...
export const usePermissionStore = defineStore('permission', () => {
   ...
   * @param type 是否是重写路由
   */
  const filterAsyncRouter = (asyncRouterMap: RouteRecordRaw[], lastRouter?: RouteRecordRaw, type = false): RouteRecordRaw[] => asyncRouterMap.filter(route => {
  ...
  const filterChildren = (childrenMap: RouteRecordRaw[], lastRouter?: RouteRecordRaw): RouteRecordRaw[] => {
    ...
    for (const element of childrenMap) {
      element.path = lastRouter ? lastRouter.path + '/' + element.path : element.path;
      if (element.children?.length && element.component?.toString() === 'ParentView') {
        children = children.concat(filterChildren(element.children, element));
      } else {
...
export const loadView = (view: any, name: string) => {
  ...
  for (const path in modules) {
    ...
    let dir = path.slice(Math.max(0, viewsIndex + 7));
    dir = dir.slice(0, Math.max(0, dir.lastIndexOf('.vue')));
    if (dir === view) {
